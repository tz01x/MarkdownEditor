...
import { validatePDFOptions, generatePDFFromElement, downloadPDF } from '../pdfUtils';
...
describe('pdfUtils', () => {
  ...
  describe('validatePDFOptions', () => {
    ...
    it('should throw error for invalid filename', () => {
      expect(() => validatePDFOptions({ filename: '' })).toThrow('Filename must be a non-empty string');
      expect(() => validatePDFOptions({ filename: null as any })).toThrow('Filename must be a non-empty string');
    });

    it('should throw error for invalid quality', () => {
      expect(() => validatePDFOptions({ filename: 'test', quality: 0.05 })).toThrow('Quality must be between 0.1 and 1');
      expect(() => validatePDFOptions({ filename: 'test', quality: 1.5 })).toThrow('Quality must be between 0.1 and 1');
    });

    it('should throw error for invalid scale', () => {
      expect(() => validatePDFOptions({ filename: 'test', scale: 0.1 })).toThrow('Scale must be between 0.5 and 5');
      expect(() => validatePDFOptions({ filename: 'test', scale: 10 })).toThrow('Scale must be between 0.5 and 5');
    });

    it('should throw error for invalid page size', () => {
      expect(() => validatePDFOptions({ filename: 'test', pageSize: 'invalid' as any })).toThrow('Page size must be a4, letter, or legal');
    });

    it('should throw error for invalid orientation', () => {
      expect(() => validatePDFOptions({ filename: 'test', orientation: 'invalid' as any })).toThrow('Orientation must be portrait or landscape');
    });
  });
