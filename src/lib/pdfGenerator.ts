...
export class BackgroundPDFGenerator {
  ...
  async generatePDF(
    ...
    try {
      ...
      const originalStyles = this.prepareElementForRendering(
        element,
        pageWidth,
        margin
      );
  ...
  private prepareElementForRendering(
    element: HTMLElement,
    pageWidth: number,
      ...
      setTimeout(() => {
        html2canvas(element, {
          ...
          onclone: clonedDoc => {
            ...
            const clonedElement =
              clonedDoc.querySelector(".pdf-export-mode") ||
    ...
    return new Promise((resolve, reject) => {
      ...
      const addNextPage = () => {
        ...
        this.updateProgress(
          "generating",
          pageProgress,
          `Generating page ${currentPage + 1} of ${totalPages}...`
        );
        ...
        img.onload = () => {
          try {
            pageCtx.drawImage(
              ...
              sourceY,
              imgWidth,
              currentPageHeightPx, // Source rectangle
              0,
              0,
              imgWidth,
              currentPageHeightPx // Destination rectangle
            );
  ...
  private updateProgress(
    stage: PDFGenerationProgress["stage"],
    progress: number,
    message: string
  ) {
